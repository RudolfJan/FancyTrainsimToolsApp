...
namespace Assets.Library.Models
  ...
  public class RouteAssetsDatabaseCollectionModel
      ...
      await Task.Run(() =>
        ...
        Parallel.ForEach<RouteModel>(routesList, async (route) =>
          ...
          if (string.IsNullOrEmpty(route.Pack))
            {
            var binList = await LoadUnpackedRouteBinFilesToListAsync(routesBasePath, route);
    ...
    public static async Task<List<FileInfo>> LoadUnpackedRouteBinFilesToListAsync(string routesBasePath,
      RouteModel route)
      {
